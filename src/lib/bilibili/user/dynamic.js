...
let deal = async (ctx) => {
	...
	// 去重：基于内容唯一标识，保留第一次出现的条目
	// 对于视频类型(av)，使用bvid；其他类型使用guid
	let seenItems = new Set();
	let uniqueItems = [];
	for (let item of items) {
		let uniqueId;
		if (item.category === 'av') {
			// 视频类型：从link中提取bvid作为唯一标识
			uniqueId = item.link.match(/\/video\/(BV[a-zA-Z0-9]+)/)?.[1] || item.guid;
		} else {
			// 其他类型：使用guid
			uniqueId = item.guid;
		}
		if (!seenItems.has(uniqueId)) {
